...
//			logger.info(href + "--该链接已经访问过---");
	...
	public User parseUserdetail(Document doc){
		...
		try {
			u.setUsername(doc.select(".title-section.ellipsis .name").first().text());//用户名
//			if(doc.select(".title-section.ellipsis .bio")!=null)
//			{
//				u.setPosition(doc.select(".title-section.ellipsis .bio").first().text());//职位
//			}
//			else
//			{
//				u.setPosition(getUserinfo(doc,"position"));//职位
//			}
		...
		//add start
		u.setAsks(Integer.valueOf(doc.select(".profile-navbar.clearfix span.num").first().text()));//提问数目
//		u.setAsks(Integer.valueOf(doc.select("profile-navbar.clearfix [href*=/path/]").first().nextSibling().));//回答数目
		//add end
		String sexStr=doc.select(".zm-profile-header-op-btns.clearfix .zg-btn.zg-btn-follow.zm-rich-follow-btn").first().text();
		if(sexStr.contains("她"))
		{
			u.setSex("女");
		}
		else if(sexStr.contains("他"))
		{
			u.setSex("男");
		}
